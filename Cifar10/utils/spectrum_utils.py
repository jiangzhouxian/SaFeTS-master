...
#Converting numpy arrays to grayscale tensor
...
def phase_spectra(images):
    ...
    # Applying the Fourier Transform to Obtain a Spectrum
...
def high_pass_filter(images):
    ...
    # Defining a Sobel filter
    sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    sobel_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)

    # Appling high-pass filter
...
#Compute the saliency map of quaternion Fourier
